 ...
 */
class TprmAssessmentsController extends AppController
{
	...
	public function rcmapping($aid, $sub_type, $rbid=null){
		...
		if($this->Auth->user('role')=='Employee'){
			if($this->isEmployeeAssessment($aid)==false){
				$this->Flash->error("Sorry! The Assessment does not belongs to logged in user.");
				return $this->redirect(['controller'=>'TprmAssessments','action'=>'tracking']);
			}
		}
		...
		if($this->request->is(['put','post'])){
			...
			return $this->redirect(['controller'=>'TprmAssessments','action'=>'view',$aid, $sub_type]);
		...
		if($sub_type=='Regulated'){
			...
			if($rcs>0){
				...
				return $this->redirect(['controller'=>'TprmAssessments','action'=>'rcmapping',$aid,$sub_type,$rbid]);
			}
	...
	public function controlMaturityRating($id=null,$asub_type=null){
		...
		if($this->request->is(['post','put'])){
			...
			$this->redirect(['controller'=>'TprmAssessments','action'=>'view',$thisAssessment->id,$asub_type]);
		}
	...
	public function assessmentRequest($invitation_id=null,$reguBodyId=null){
		...
		$invitations = $this->Invitations
			->find()
			->where(['id' => $invitation_id, 'company_email' => $this->Auth->user('email')])
			->first();

		if ($invitations) {
			$assessment_type =  $invitations->assessment_type;
		} else {
			echo "Invitation not found."; // Add error handling if the invitation is not found
			die;
		}
		...
		if($this->request->is(['post'])){
			...
			$invitations->assessment_status = 1;
			$this->Invitations->save($invitations);
		...
		$this->set(compact('fRisks'));
		$this->set(compact('assessment_type'));

	            	...
	            	<?php
	            		if($asub_type=='Regulated'){
	            			echo $this->Html->link('View Assessments Details',array(
								'controller'=>'TprmAssessments','action'=>'view',$aControl->assessments_regulatory_body->assessment_id,$asub_type
							));
	            		} else {
	            			echo $this->Html->link('View Assessments Details',array(
								'controller'=>'TprmAssessments','action'=>'view',$aControl->assessment_id,$asub_type
							));
	            		}
...
<div class="card" style="border:1px solid #ccc;">
	...
	<table class="table table-bordered table-hover">
		...
		<tbody>
			<?php foreach($aControl->assessment_control_requirements as $creq): ?>
				<tr>
					...
					<td style="vertical-align: top;">
						...
						<textarea class="form-control input-sm instantSaveRef" style="white-space:pre-line;" data-table="assessment_control_requirements" data-id="<?php echo $creq->id; ?>"><?= $creq->reference ?></textarea>
					</td>
				...
				<tr>
					...
					<td class="p-0">
						<table class="table table-borderless m-b-0">
							<tbody>
								<?php foreach($aControl->assessment_maturity_scores as $mAttribute): ?>
									<tr>
										<td>
	...
	$(function(){
		...
		$(document).on('click','.deleteArtifact',function(){
			...
			if(confirm("Are you sure to delete this Artifact.")){
				//console.log(file);
				finput.parent('td').find('.falert').remove();
				var thisUrl = "<?php echo $this->Url->build(array('controller'=>'TprmAssessments','action'=>'deleteArtifactFromAdmin'),true); ?>";
				thisUrl = thisUrl.replace("http:", rproto);
				$.ajax({
					url : thisUrl,
					method : "POST",

	...
	<body style="font-family:Arial, Helvetica, sans-serif;">
		<div id="report" class="report" style="">
			...
			<ul style="list-style:none;">
				...
				<li>
					...
					<ul style="list-style:none;">
						<?php $sr=0; foreach($assessment->ffiec_assessment_risks as $maRisk): ?>
				...
				<li>
					...
					<ul style="list-style:none;">
						<?php $sr=1; foreach($assessment->ffiec_assessment_domains as $maControl): ?>
			...
			<p>
				...
				<?php
					$noOfReqs = 0;
					foreach($assessment->ffiec_assessment_domains as $ac){
						foreach($ac->ffiec_assessment_domain_a_factors as $af){
							$noOfReqs+=count($af->ffiec_assessment_domain_requirements);
						}
					}
					echo $noOfReqs;
				?>

				 specific declarative statements  as
				...
				<?php
					...
					foreach($assessment->ffiec_assessment_risks as $ar){
				...
				<table style="" cellspacing="0" cellpadding="5" border="1" bordercolor="#222222" width="100%">
					...
					<tbody>
						<?php foreach($assessment->ffiec_assessment_risks as $ar): ?>
			...
			<p>
				<?php
					$noOfNonCompliance=0;
					foreach($assessment->ffiec_assessment_domains as $acs){
						foreach($acs->ffiec_assessment_domain_a_factors as $racsr){
							foreach($racsr->ffiec_assessment_domain_requirements as $acsr){
			...
			<table style="" cellspacing="0" cellpadding="5" border="1" bordercolor="#222222" width="100%">
				...
				<tbody>
					<?php foreach($assessment->ffiec_assessment_domains as $acs): ?>
						...
						<tr>
							...
							<td>
								<ul>
									<?php foreach($acs->ffiec_assessment_domain_a_factors as $racsr): ?>
										<li>
											<?php echo $racsr->name; ?>
											<ul>
												<?php foreach($racsr->ffiec_assessment_domain_requirements as $acsr): ?>
		...
		<h3>1.2 <u>Table of Findings</u></h3>
				...
				<table width="100%" border="1" bordercolor="#333" cellspacing="0" style="font-size:10px;">
					<thead>
						<tr>
							...
							<th style="text-align:center;" colspan="<?php echo count($assessment->ffiec_assessment_domains); ?>"  class="text-dark">FFIEC Domains</th>
						</tr>
						<tr>
							...
							<?php foreach($assessment->ffiec_assessment_domains as $eCont): ?>
			...
			<p align="justify">
				...
				policies and procedures. A total of <?php echo count($assessment->ffiec_assessment_risks); ?> risk areas
				were assessed and mapped to <?php echo count($assessment->ffiec_assessment_domains); ?>, in addition
			...
			<p align="justify">
				The Analysis considered logical controls in <?php echo count($assessment->ffiec_assessment_risks); ?>
				Risk Domains for the <?php echo $company->company_name; ?> environment.
			</p>

			<?php $sr=0; foreach($assessment->ffiec_assessment_risks as $aRisk): ?>
				<h4>3.<?php echo $sr+=1; ?> <?php echo $aRisk->risk; ?></h4>
				<h5>Overview</h5>
				<p align="justify">
					<?php echo strlen($aRisk->description)>2?$aRisk->description:"<i>-No overview available-</i>"; ?>
				</p>
				<h5>Inherent Risk</h5>
				<p align="justify">
			...
			<p align="justify">
				The Analysis considered controls in <?php echo count($assessment->ffiec_assessment_domains); ?>
			...
			<p align="justify">
				...
				<?php echo count($assessment->ffiec_assessment_domains); ?> broad, yet discreet, Control Domains.
			...
			<?php $sr=1; foreach($assessment->ffiec_assessment_domains as $aControl): ?>
				...
				<table width="100%" border="1" cellspacing="0" cellpadding="3" style="border:1px solid #333;border-width:0 0 1px 1px">
					<thead>
						<tr>
							...
							<?php foreach($aControl->ffiec_assessment_maturity_scores as $amScore): ?>
	    	  ...
	    	  <table cellspacing="0" cellpadding="2" style="font-size:15px;border:1px solid;border-width:1px 1px 0 0;">
			  	...
			  	<tbody>
			  		...
			  		<?php $i=1; foreach($table as $risk_id=>$rows): ?>
				  		<tr>
				  			<td style="background:#800080;color:#ffffff;text-align:left;border:1px solid #333;border-width:0 0 1px 1px"><?php echo $risk_id; ?></td>

                        ...
                        <div class="row cisobpadding tool-form-submission">
                            ...
                            <div class="col-md-3 col-sm-3 col-md-3 col-xs-12">
                            	<?php
                            		echo $this->Form->control('atype',[
                            			...
                            			'options'=>array('Self'=>'Self Assessment'),
                            			'required'=>true
                            		]);
                            	?>
                            </div>
                            ...
                            <div class="col-md-3 col-sm-3 col-md-3 col-xs-12 m-t-10">
                            	<?php
									...
									$options = [
										'' => '-- Select --',
										'Generalized' => 'Generalized Assessment',
										'Regulated' => 'Regulated Assessment',
										'FFIEC Regulated' => 'FFIEC Regulated Assessment',
									...
									echo $this->Form->control(
										...
										[
											...
											'type' => 'select',
											'options' => array_merge(['' => '-- Select --'], array_intersect_key($options, array_flip([$cp]))),
											'required' => true
										]
									);
                            	?>
  	...
  	$(function(){
  		...
  		function updateInherent(element){
  			...
  			if(isAll==element.find('.ffIRiskLevelSelect').length){
  				var thisUrl = "<?php echo $this->Url->build(array('controller'=>'TprmAssessments','action'=>'getRiskScaleByScore2'),true); ?>";
...
$(document).ready(function() {
   function triggerSubtypeChange() {
		var selectedOption = "<?php echo $cp; ?>";
		$('.subtype').val(selectedOption);
      ...
      if ($('.atype').val()) {
         ...
         if (subtypeval) {
            ...
            var thisUrl = "<?php echo $this->Url->build(array('controller'=>'TprmAssessments','action'=>'getRisksAndControls'),true); ?>";
            thisUrl = thisUrl.replace("http:", rproto);
   ...
   triggerSubtypeChange(); // Call the function on page load
});
    	...
    	$(function(){
			...
			$(document).on('click','.thisRegulatoryBodies .reguBody',function(){
				...
				if(rbody.prop('checked')==true){
					...
					var thisUrl = "<?php echo $this->Url->build(array('controller'=>'TprmAssessments','action'=>'getRisksAndControlsRegulated'),true); ?>";
					thisUrl = thisUrl.replace("http:", rproto);

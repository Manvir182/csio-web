					    				...
					    				<div class="col-sm-4">
					    					<?php
							    				echo $this->Html->link('<i class="fa fa-calendar"></i> Update Risk Control Mapping',[
							    					'controller'=>'TprmAssessments','action'=>'rcmapping',$assessment->id,$assessment->sub_type,$rBody->id
							    				],[
				    							...
				    							<div class="col-sm-4">
				    								<?php
						    							echo $this->Html->link('<i class="fa fa-bookmark"></i> Perform Control & Maturity Assessment',[
						    								'controller'=>'TprmAssessments','action'=>'controlMaturityRating',$control->id,$assessment->sub_type
						    							],[
	    					...
	    					<div class="col-3 text-right">
	    						<?php
				    				echo $this->Html->link('<i class="fa fa-calendar"></i> Risk Control Mapping',[
				    					'controller'=>'TprmAssessments','action'=>'ffiecRcmapping',$assessment->id
				    				],[
			    					...
			    					<div class="col-3 text-right">
			    						<?php
			    							echo $this->Html->link('<i class="fa fa-bookmark"></i> Perform Control Assessment',[
			    								'controller'=>'TprmAssessments','action'=>'domainMaturityRating',$fdomain->id,$assessment->sub_type
			    							],[
			    								'class'=>'btn btn-sm btn-danger float-right m-b-0',
			    								'escape'=>false,
			    								//'onclick'=>'return false;'
	    			...
	    			<h4 class="m-b-0 p-10 bg-info text-white font-bold">
	    				<div class="row">
		    				...
		    				<div class="col-sm-4">
			    				<?php
				    				echo $this->Html->link('<i class="fa fa-calendar"></i> Update Risk Control Mapping',[
				    					'controller'=>'TprmAssessments','action'=>'egrcRcmapping',$assessment->id,$assessment->sub_type
				    				],[
				    					'class'=>'btn btn-warning btn-sm float-right',
				    					'escape'=>false,
				    					'disabled'=>false
	    					...
	    					<h5 class="m-b-0 p-10 bg-light text-dark font-bold" style="border-bottom:1px solid #ccc;">
	    						<div class="row">
	    							...
	    							<div class="col-sm-4">
	    								<?php
			    							echo $this->Html->link('<i class="fa fa-bookmark"></i> Perform Control & Maturity Assessment',[
			    								'controller'=>'TprmAssessments','action'=>'policyMaturityRating',$control->id,$assessment->sub_type
			    							],[
			    								'class'=>'btn btn-sm btn-danger float-right m-b-0',
			    								'escape'=>false,
			    								'disabled'=>true
	    			...
	    			<h4 class="m-b-0 p-10 bg-info text-white font-bold">
	    				<div class="row">
		    				...
		    				<div class="col-sm-4">
			    				<?php
				    				echo $this->Html->link('<i class="fa fa-calendar"></i> Update Risk Control Mapping',[
				    					'controller'=>'TprmAssessments','action'=>'rcmapping',$assessment->id,$assessment->sub_type
				    				],[
...
<script>
	...
	$(function(){
		...
		<?php if($assessment->status!='Completed'): ?>
			...
			$(document).on('change','.instantSaveFactor',function(){
				...
				if(field.val()){
					...
					var thisUrl = "<?php echo $this->Url->build(array('controller'=>'TprmAssessments','action'=>'saveInstantFactor'),true); ?>";
			...
			$(document).on('change','.instantSave',function(){
				...
				if(field.val()){
					...
					//console.log(postData);
					var thisUrl = "<?php echo $this->Url->build(array('controller'=>'TprmAssessments','action'=>'saveInstantUpdate'),true); ?>";
					thisUrl = thisUrl.replace("http:", rproto);
					$.ajax({
						url : thisUrl,
						method : "POST",
			...
			$(document).on('change','.artifact',function(){
				...
				if(jQuery.inArray(ext, ['pdf']) == -1){
					...
					var thisUrl = "<?php echo $this->Url->build(array('controller'=>'TprmAssessments','action'=>'uploadArtifactFromAdmin'),true); ?>";
					thisUrl = thisUrl.replace("http:", rproto);
					$.ajax({
						url : thisUrl,
						method : "POST",
		...
		$(document).on('click','.showResult',function(){
			...
			var win = window.open("<?php echo $this->Url->build(array('controller'=>'TprmAssessments','action'=>'viewResult'),true); ?>/"+aid+"/"+st,"Assessment Results","width=1200,height=550,left=0,top=0");
...
<script>
	...
	$(function(){
		...
		var thisUrl2 = "<?php echo $this->Url->build(array('controller'=>'TprmAssessments','action'=>'getRiskScaleByScore2'),true); ?>";
		...
		$(document).find('.fcTargetBtnDropdown').on('focus',function(){
			...
			var mlevel = sbox.val();
			var thisUrl2 = "<?php echo $this->Url->build(array('controller'=>'TprmAssessments','action'=>'updateFfiecControlFactor'),true); ?>";
			thisUrl2 = thisUrl2.replace("http:", rproto2);

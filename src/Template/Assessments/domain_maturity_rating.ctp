    ...
    <div class="col-md-12 col-12">
        <ol class="breadcrumb">
        	<?php if($thisUser['role']=='Employee'): ?>
	            ...
	            <li class="breadcrumb-item active">Perform Domain &amp; Maturity Assessment</li>
        	<?php else: ?>
	            <li class="breadcrumb-item">
	            	<?php 
	            		echo $this->Html->link('Dashboard',array(
...
<div class="card" style="border:1px solid #ccc;">
	<div class="card-header">
		<h4 class="m-b-0 p-b-0">
			Update Compliance Statuses for <span class="text-danger"><?= $aControl->name ?></span> Declarative Statements
		</h4>
	</div>
	<table class="table table-bordered table-hover m-0">
		<thead>
			<tr>
				<th class="bg-light text-themecolor">Assessment Factor</th>
				<th class="bg-light text-themecolor" style="width:40% !important;">Declarative Statement</th>
				<th class="bg-light text-themecolor" style="width:200px !important;">Artifact File</th>
				<th class="bg-light text-themecolor" style="width:25% !important;">Refs/Narrative</th>
				<th class="bg-light text-themecolor">
					Compliance Status
		...
		<tbody>
			<?php foreach($aControl->ffiec_assessment_domain_a_factors as $aFactor): ?>
				
				<?php $af=0; foreach($aFactor->ffiec_assessment_domain_requirements as $creq): ?>
					<tr>
						<?php if($af==0): ?>
							<td rowspan="<?php echo count($aFactor->ffiec_assessment_domain_requirements); ?>">
								<?php echo $aFactor->name; ?>
							</td>
						<?php endif; ?>
						<td <?php echo strlen($creq->name)>255?'style="white-space:pre-line;"':''; ?>><?= $creq->name ?></td>
						...
						<td style="vertical-align:top;">
							<textarea class="form-control input-sm instantSaveRef" style="white-space:pre-line;" data-table="ffiec_assessment_domain_requirements" data-id="<?php echo $creq->id; ?>"><?= $creq->reference ?></textarea>
						</td>
						<td style="vertical-align:top;">
							<select class="form-control rComp" name="rCompliance[<?= $aFactor->id ?>][<?= $creq->id ?>]">
...
<div class="card" style="border:1px solid #ccc;">
	<div class="card-header">
		<h4 class="m-b-0 p-b-0">
			Update Maturity Ratings for FFIEC Doman
	...
	<div class="card-body p-0">
		<table class="table table-bordered m-0">
			...
			<tbody>
				<tr>
					<td style="width:40%;">
						<select class="form-control cComp" name="cStatus">
							<option value="">--Select--</option>
							<?php foreach($compStatuses as $cStatus): ?>
					...
					<td class="p-0">
						...
						<table class="table table-borderless m-b-0">
							<tbody>
								...
								<?php foreach($aControl->ffiec_assessment_maturity_scores as $sbkey=>$mAttribute): ?>
									<tr>
										<td>
										...
										<td>
											...
											<input type="hidden" name="mOptions[<?= $mAttribute->id ?>]" class="mAttrOptionField mAttr" value="<?= $selectedMO ?>">
											<!--
											<select name="mOptions[<?= $mAttribute->id ?>]" required class="form-control mAttr">
												<option title="Please Select One Option" value="">-- Select --</option>
												<?php foreach($mAttributeOptions as $mOption): ?>
													<option title="<?php echo $ttips[$mOption->name]; ?>" <?= $mOption->name!=$mAttribute->maturity_option?:"selected" ?> value="<?php echo $mOption->name; ?>~<?php echo $mOption->score; ?>">
														<?php echo $mOption->name; ?>
													</option>
												<?php endforeach; ?>
											</select>
...
<div class="fixed-bottom bg-light p-2 text-center">
	...
	<button class="btn btn-success submitBtn mr-5" type="submit" <?php echo $thisAssessment->status=='Completed'?"disabled":''; ?>>
		<i class="fa fa-check mr-2"></i>
		Update Now
	</button>
	<?php //if($aControl->compliance_status==""): ?>
	 	<button class="btn btn-warning submitBtn draft" type="submit" <?php echo $thisAssessment->status=='Completed'?"disabled":''; ?>>
	...
	$(function(){
		...
		$(document).on('submit','.cmrForm',function(frm){
			...
			if($('#submitType').val()=='update'){
				...
				if(flag==false){
					alert("Kindly update all fields.");
					frm.preventDefault();
				}
			}
			///}
		...
		$(document).on('change','.artifact',function(){
			...
			if(jQuery.inArray(ext, ['pdf']) == -1){
				...
				var thisUrl = "<?php echo $this->Url->build(array('controller'=>'assessments','action'=>'uploadArtifactForFfiec'),true); ?>";
				...
				$.ajax({
					...
					success : function(data) {
						...
						if(data==3){
							...
							$('<button class="btn btn-sm btn-danger deleteArtifact" type="button" ><i class="fa fa-times"></i></button>').insertBefore(finput);
		...
		$(document).on('click','.deleteArtifact',function(){
			...
			if(confirm("Are you sure to delete this Artifact.")){
				//console.log(file);
				finput.parent('td').find('.falert').remove();
				var thisUrl = "<?php echo $this->Url->build(array('controller'=>'assessments','action'=>'deleteArtifactFfiec'),true); ?>";
				thisUrl = thisUrl.replace("http:", rproto);
				$.ajax({
					url : thisUrl,
					method : "POST",
		...
		$(document).on('change','.cComp, .rComp',function(){
			...
			thisComp = thisComp.substring(0,thisComp.indexOf('~'));
			var msg = 'You are not able to conclude "'+thisComp+'" ';
				msg+=' for Control Area "<?= $aControl->name ?>" because some control requirements are  or';; 
				msg+=' . Please review Compliance status of domain declarative statements or select'; 
				msg+=' appropriate Overall domain compliance status of Partially Compliant or Non-compliant.';

...
<div class="">
	...
	<table class="table table-bordered bg-white exporttable">
		<tr>
			<td>
	...
	<button class="btn btn-primary export" data-target="toExport">
		Export All
	</button>
...
<div class="tab-content" id="pills-tabContent">
  <div class="tab-pane fade show active" id="risks" role="tabpanel" aria-labelledby="risks-tab">
			    ...
			    <div id="rcollapse<?= $rBody->id ?>" class="collapse <?php echo $i==0?'show':''; ?>" aria-labelledby="headingr<?= $rBody->id ?>" data-parent="#rbAccordion">
			      <div class="card-body p-0">
			        <div class="row">
				    	<div class="col-12">
				    		<div class="card">
				    			<button style="width:80px;" class="btn btn-xs btn-warning exportbtn" data-target="#<?php echo clean($rBody->regulatory_body->name); ?>_Risks">Export</button>
  		
				    			<table class="table table-bordered m-b-0 table-hover" id="<?php echo clean($rBody->regulatory_body->name); ?>_Risks">
				    				<thead>
				    					<tr>
  	...
  	<?php else: ?>
  		<button class="btn btn-xs btn-warning exportbtn" data-target="#Risks">Export</button>
  		<table class="table table-bordered m-b-0 table-hover gRisks" id="Risks">
			<thead>
  ...
  <div class="tab-pane fade" id="controls" role="tabpanel" aria-labelledby="controls-tab">
			        ...
			        <div class="row">
				    	<div class="col-12">
				    		<div class="card bg-white" style="border:1px solid #ccc;" id="assessmentControls">
				    			<button style="width:80px;" class="btn btn-xs btn-warning exportbtn" data-target="#<?php echo clean($rBody->regulatory_body->name); ?>_Controls">Export</button>
				    			<table class="table table-bordered table-hover m-b-0" id="<?php echo clean($rBody->regulatory_body->name); ?>_Controls">
			    					<thead>
  	...
  	<?php else: ?>
  		<button class="btn btn-xs btn-warning exportbtn" data-target="#Controls">Export</button>
  		<table class="table table-bordered table-hover m-b-0" id="Controls">
			<thead>
				<tr class="text-center text-blue">
					<td rowspan="2" valign="middle" class="bg-info text-white"></td>
					<td rowspan="2" valign="middle" class="bg-info text-white">Control Area</td>
					...
					<tbody>
						...
						<?php $crl=0; foreach($control->assessment_control_requirements as $creq): ?>
    						<tr style="display:none;" class="contReqslist table-info contReqTable<?= $control->id ?>">
    							<?php if($crl==0): ?>
    								<td rowspan="<?php echo $rLength; ?>"></td>
    								<td rowspan="<?php echo $rLength; ?>"> <?= $control->name ?> Requirements </td>
    							<?php endif; ?>
    							<td colspan="<?php echo count($assessment->assessment_controls[0]->assessment_maturity_scores)+2; ?>">
    								<?= $creq->name ?> <br>
    								<?php if(strlen($creq->artifact)>0): ?>
    									<button class="btn btn-sm btn-warning showArtifact" type="button" data-file="<?php echo $creq->artifact; ?>">
    										Artifact
  ...
  <div class="tab-pane fade" id="rcmapping" role="tabpanel" aria-labelledby="rcmapping-tab">
					    ...
					    <div id="mcollapse<?= $rcmap->id ?>" class="collapse <?php echo $a==0?'show':""; ?>" aria-labelledby="mheading<?= $rcmap->id ?>" data-parent="#mapingAccordion">
					      <div class="card-body p-0">
					       		<div class="table-responsive">
					       		  <button style="width:80px;" class="btn btn-xs btn-warning exportbtn" data-target="#<?php echo clean($rBody->regulatory_body->name); ?>_RCMapping">Export</button>
				    			
						    	  <table class="table table-bordered table-hover myTable m-0" id="<?php echo clean($rBody->regulatory_body->name); ?>_RCMapping">
  		...
  		<?php else: ?>
  			...
  			?>
  			<div class="table-responsive">
  				<button class="btn btn-xs btn-warning exportbtn" data-target="#RCMapping">Export</button>
	    	  <table class="table table-bordered table-hover myTable m-0 gMappings" id="RCMapping">
  ...
  <div class="tab-pane fade" id="scales" role="tabpanel" aria-labelledby="scales-tab">
  	  <h3>Risk Severity</h3>
  	  <button class="btn btn-xs btn-warning exportbtn" data-target="#RiskSeverityScale">Export</button>
  	  <table class="table table-bordered table-hover" id="RiskSeverityScale" style="font-size:15px;margin-bottom:0px;">
    	<thead>
    ...
    <button class="btn btn-xs btn-warning exportbtn" data-target="#ControlEffectiveness">Export</button>
  	  
    <table class="table table-bordered table-hover" id="ControlEffectiveness">
    	<thead>
    		<tr class="table-inverse">
    			<th>Name</th>
    			<th>Description</th>
